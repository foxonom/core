...
```php
public string Headzoo\Core\Obj::getClassName()
```
...
global namespace.
```php
public string Headzoo\Core\Obj::getNamespaceName()
```
...
```php
protected mixed Headzoo\Core\Obj::toss(string $exception, string $message, int $code)
```
...
```php
private string Headzoo\Core\Obj::interpolate(string $message, array $context)
```
